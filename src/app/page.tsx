...
export default function Home() {
  ...
  return (
    <>
      <header className="flex items-center justify-between max-w-5xl w-full my-0 mx-auto py-6">
        {/* Images must have alternative text  */}
        <Image src={LogoImg} width={286 / 2} alt="Rocketseat Blog"/>
        <nav>
          {/* Links must have discernible text */}
          <a href="https://github.com/brunoduartelima" aria-label="Github" style={{color: "#8257e5"}}>
            <svg 
              stroke="currentColor"
              fill="currentColor"
              strokeWidth={0}
      ...
      <main>
        <div tabIndex={0} className="text-gray-400 max-w-5xl w-full my-0 mx-auto py-6 space-y-5">
      ...
      <footer className="flex items-center justify-between max-w-5xl w-full my-0 mx-auto py-6">
        <Image src={LogoImg} width={286 / 2} alt="Rocketseat Blog"/>
        <nav aria-label="Informações adicionais">
          <button 
            type="button"
            aria-controls='modal'
            className="text-[#8c65e7] bg-gray-500 p-2 rounded bg-opacity-10"
      ...
      >
        <div className="bg-white p-4 rounded space-y-5">
          ...
          <p id="modalDescription">
            Nos envie uma mensagem para que possamos aprimorar nosso conteudo.
          </p>
          <fieldset className="flex flex-col">
            <label id="nameLabel" htmlFor="name" className="text-left font-bold">Nome completo</label>
            <input 
              aria-labelledby="nameLabel"
              type="text"
              name="name"
              className="border border-black rounded outline-2 outline-purple-500"
            />
          </fieldset>
          <fieldset className="flex flex-col">
            <label id="messageLabel" htmlFor="message" className="text-left font-bold">Mensagem</label>
            <textarea 
          ...
          <button 
            type="button"
            className="bg-[#8257e5] font-bold text-white rounded p-2 w-28 hover:opacity-75"
